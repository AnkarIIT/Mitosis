# NEET Mitos App — Deep Research & Audit (Cited Draft)
...
**Status:** CITED — draft verified; one prior claim corrected after re-inspection
...
**Weaknesses**
- Two parallel provider files (`core/provider/providers.dart` and `core/providers/providers.dart`) exist and are **not identical**; this is a merge/refactor artifact that creates ambiguous import risk.
...
- `QuestionPaperGenerator` imports `getAllQuestions()` from `neet_sample_data.dart` (verified at `lib/core/constants/neet_sample_data.dart:911`); the earlier draft incorrectly flagged this as missing.
...
- `neet_sample_data.dart`: ~919 lines; contains `biologyQuestions`, `chemistryQuestions`, `physicsQuestions`, and `getAllQuestions()` at line 911.
...
- `QuestionPaperGenerator._removeYearMarking()` strips 4-digit years from `ncertReference` using regex (`RegExp(r'\d{4}')`), which can corrupt legitimate text containing 4-digit numbers.
...
**Recommended path:**
1. Create a `NcertCatalog` — a Dart map from topic IDs to asset paths, derived from the naming convention above.
...
|-----------|-------|--------|
| Total Questions | 180 | NTA notice, Careers360, Jagranjosh |
| Total Marks | 720 | NTA notice, Careers360, Jagranjosh |
| Duration | 180 minutes (3 hours) | Careers360, Jagranjosh |
| Mode | Pen-and-paper (offline) | Jagranjosh, Shiksha |
| Correct Answer | +4 marks | Times Now, Free Press Journal |
| Wrong Answer | -1 mark | Times Now, Free Press Journal |
| Unanswered | 0 marks | Standard NEET scheme |
| Physics | 45 questions | Shiksha, Vedantu |
| Chemistry | 45 questions | Shiksha, Vedantu |
| Biology | 90 questions (45 Botany + 45 Zoology) | Shiksha, Vedantu |
...
- Shiksha exam pattern breakdown.
- Vedantu NEET exam pattern.
...
**Sources:**
- Careers360: "NMC NEET UG Syllabus 2026 Unchanged: No Addition or Reduction"
- NTA official syllabus PDF notice.
...
**Sources:**
- Darwin: https://darwin.mcqdb.com/
- Unprep: https://play.google.com/store/apps/details?id=com.unbind&hl=en
- Yukthis: https://apps.apple.com/us/app/yukthis-neet/id6760161721
...
1. **Duplicate provider files**
   - `lib/core/provider/providers.dart` and `lib/core/providers/providers.dart` both exist and differ significantly (verified by `diff`). This can cause ambiguous imports or silent provider override bugs.

2. **Cloud sync pull is unimplemented**
   - `_syncQuizAttempts()` in `cloud_sync_service.dart` pulls cloud rows into a local list `cloudAttempts` and iterates with an empty body (`for (var _ in cloudAttempts) { /* Future: ... */ }`). Pull does not actually merge.

3. **ML tokenizer is dummy code**
   - `MLService._tokenize()` converts text to `text.codeUnitAt(i)`. This is not a tokenizer; it produces character-code embeddings with no vocabulary. The TFLite model `ml/sentence_encoder.tflite` is also not visible in the inspected file tree, so the service silently falls back to word-overlap similarity.

4. **Rank predictor is a static hardcoded table**
   - `RankPredictor.predictRank()` uses fixed score→rank mappings labeled "2023-2024 trends". For 2026, this is stale and misleading. A real predictor would use percentile data from the specific exam session.

### 7.2 Medium Priority

5. **Year-marker regex strips legitimate text**
   - `_removeYearMarking()` uses `replaceAll(RegExp(r'\d{4}'), '')`. This removes any 4-digit number, which could corrupt legitimate content.

6. **Gemini API key friction**
...
- Supabase RLS is defined in `supabase/01_content_catalog.sql`, but the app does not appear to enforce row-level user isolation in all queries.
...
1. **Fix fatal bugs:** Duplicate provider file cleanup, cloud sync pull loop, regex year-marker edge cases.
...
- NMC/UGMEB syllabus notice: https://nta.ac.in/Download/Notice/Notice_20260108180635.pdf
